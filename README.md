# WhooshingLander

Vibe coder. Builder. Curious human. **@whooshinglander everywhere**

![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=flat&logo=tailwindcss&logoColor=white)
![Claude](https://img.shields.io/badge/Claude-D97757?style=flat)
![Gemini](https://img.shields.io/badge/Gemini-4285F4?style=flat&logo=google&logoColor=white)

I build useful things on the internet, mostly fast-moving web products, AI tools, and niche sites with real users.

## Current Projects

- 📈 **[Option Profit Calculator](https://optionprofitcalculator.ai)** - Free options strategy calculator for exploring profit, loss, breakeven, and payoff structure.
- 🧾 **[SG Tenancy Agreement](https://sgtenancy.com)** - Singapore tenancy agreement generator and explainer for landlords and tenants.
- 🏦 **[Direct Index Club](https://directindex.club)** - Plain-English tools and content around ETF fees, direct indexing, and long-term investing.
- 🃏 **[PokerSnap](https://pokersnap.app)** - AI-assisted poker hand analysis and study workflow for players reviewing decisions.
- 💸 **[AskTaxes](https://asktaxes.com)** - AI-seeded tax Q&A site built to answer practical tax questions across countries.
- 🌍 **[AskVisa](https://askvisa.co)** - AI-seeded immigration and visa Q&A platform for practical country-specific questions.
- 🧖 **[ClawSpa](https://clawspa.org)** - OpenClaw tooling for deeper agent scans, diagnostics, and machine-aware workflows.
- 🏸 **[Pickle Singapore](https://picklesg.com)** - Directory and content hub for pickleball courts, clubs, and guides in Singapore.
- 🚗 **[Pickle Johor](https://picklejohor.com)** - Directory and content hub for pickleball courts and community spots in Johor.
- 🌆 **[Pickle KL](https://picklekualalumpur.com)** - Directory and content hub for pickleball in Kuala Lumpur.
- 🎾 **[Padel Singapore](https://padelsingapore.com)** - Directory and content hub for padel courts, clubs, and guides in Singapore.
- 🌴 **[Padel Johor](https://padeljohor.com)** - Directory and content hub for padel in Johor, Malaysia.

## ClawHub Skills

- ☀️ **[ClawSpa](https://clawhub.ai/whooshinglander/clawspa)** - Agent utility toolkit for deeper scans, diagnostics, and OpenClaw workflows.
- 🥋 **[Iknowkungfu](https://clawhub.ai/whooshinglander/iknowkungfu)** - Skill discovery helper for finding useful OpenClaw skills fast.
- 🌐 **[Browser Fu](https://clawhub.ai/whooshinglander/browser-fu)** - Browser-oriented workflow skill for more reliable web tasks and research.
- 🧠 **[Feelslikeclaude](https://clawhub.ai/whooshinglander/feelslikeclaude)** - Style overlay that makes OpenClaw more proactive, calm, and execution-focused.
- 🔥 **[Where Am I Burning Tokens?](https://clawhub.ai/whooshinglander/whereamiburningtokens)** - Read-only diagnostics for seeing where OpenClaw token usage and cost are actually going.
- 😴 **[Claude Says Go To Sleep](https://clawhub.ai/whooshinglander/claude-says-go-to-sleep)** - Tiny joke skill that only tells you to go to sleep, with rotating sleep instructions.

## Connect

- Email: **[whooshinglander@gmail.com](mailto:whooshinglander@gmail.com)**
- GitHub: **[@whooshinglander](https://github.com/whooshinglander)**
- Buy Me a Coffee: **[buymeacoffee.com/WhooshingLander](https://buymeacoffee.com/WhooshingLander)**
- Substack: **[@whooshinglander](https://substack.com/@whooshinglander)**
